...
vector<Move> get_valid_actions(Board& board, const map<uint64_t,pair<pair<double,chess::Move>,int>>& StaticMap) {
...
pair<double, Move> alpha_beta_pruning(Board& board, double alpha, double beta, int depth, bool max_player_flag, map<uint64_t,pair<pair<double,chess::Move>,int>>& StaticMap, int maxtimeinmilli, auto& startTime) {
    ...
    if (max_player_flag) {
        ...
        for (const Move& action : get_valid_actions(board, StaticMap)) {
...
void handleGo(int maxtimeinmilli=10000) {
    ...
    for (depth = initialDepth; depth <= maxDepth; ++depth) {
        ...
        cout<<"info depth "<<depth<<" "<<"score cp "<<bestMove.first*100<<" time "<<std::chrono::duration_cast<milliseconds>(NowTime - startTime).count()<<endl;
