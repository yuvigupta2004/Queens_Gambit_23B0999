...
double PieceDifference(const Board& board) {
    ...
    map<Piece, array<int,64>> END_SQUARE_PIECE_VALUES {
    {Piece::WHITEPAWN, {
        0, 0, 0, 0, 0, 0, 0, 0,
    ...
    int totalpieces = countPieces(board);
    for (int i=0; i<64; i++){
        int rownumber = 1 + i%8;
        ...
        double pieceatsquarevalue=0;
        if (piece == Piece::BLACKPAWN || piece == Piece::WHITEPAWN ){   
            pieceatsquarevalue = SQUARE_PIECE_VALUES[piece][squarenumber]*((totalpieces-2.0)/30.0) + END_SQUARE_PIECE_VALUES[piece][squarenumber]*((32.0 - totalpieces)/30.0);
        }
        else{
            pieceatsquarevalue = SQUARE_PIECE_VALUES[piece][squarenumber] *(1 + (32.0 - totalpieces)/40);
        }
    ...
    if (abs(white_points - black_points)<=0.001){return 0;}  
    else{return (white_points - black_points)*(1.0+(32.0-totalpieces)/40.0);}
}


// Calculate static value for a board position
double StaticValue(const Board& board) {
    double piece_diff = PieceDifference(board);
    // cout<<"No Error in PieceDifference: "<<piece_diff<<endl;

    // value += sgn(piece_diff) * pow(2, abs(piece_diff));

    // cout<<piece_diff<<endl;
    return piece_diff;
}
...
vector<Move> get_valid_actions(Board& board) {
    ...
    //         u_int64_t newboardhash = board.hash();
    //         auto it = StaticMap.find(newboardhash);

    //         if (it != StaticMap.end()) {
    //             orderedMoveswithvalue.push_back({it->second.first.first, move});
    //         }
...
pair<double, Move> alpha_beta_pruning(Board& board, double alpha, double beta, int depth, bool max_player_flag, map<uint64_t,pair<pair<double,chess::Move>,int>>& StaticMap, int maxtimeinmilli, auto& startTime) {
    ...
    if (is_terminal_history(board)) {
        ...
        pair<double, Move> ans = {double(termeval),Move()};
    ...
    if (depth == 0) { 
        ...
        double ans = double(StaticValue(board));
...
int main(){
    ...
    Board board =  Board("r1b1kb1r/pp1ppppp/8/1NQ4n/4P3/4B3/2P1KPPP/7q b - - 9 25");
    int initialDepth = 1;
    int maxDepth = 16;
    int depth = initialDepth;
    int maxtimeinmilli = 20000;

    map<uint64_t,pair<pair<double,chess::Move>,int>> StaticMap;
    pair<int, Move> bestMove;
    ...
    for (depth = initialDepth; depth <= maxDepth; ++depth) {
            ...
            std::cout<<"Reached Depth: "<<depth<<"      "<<"Eval: "<<bestMove.first<<"  "<<"Best Move: "<<bestMove.second<<"            "<<std::chrono::duration_cast<milliseconds>(NowTime - startTime).count()<<endl;
        ...
        std::cout << "My Engine's Move is: " << san_move << endl;
}
